...
class BitField(object):
    ...
    def __init__(self, from_bit: int, to_bit: int) -> None:
        ...
        self.to_bit = to_bit
        self.bitrange = to_bit-from_bit+1
        self.mask = 0
        for bit in range(self.bitrange):
    ...
    def extract_signed(self, word: int) -> int:
        ...
        print(bin(word))
        mask = 0
        for i in range(self.bitrange):
            mask = (mask << 1) | 1
        sign = bin(word)[-(self.to_bit+1)]
        field = (word>>self.from_bit) & mask
        #sign= bin(field)[2]
        if sign=="1":
            return - ((~field & mask) + 1)
        else:
    ...
    def extract_signed(self, word: int):
        ...
        field=word&mask
        mask2=0b111111111
        #print("field:",bin(field))
        sign = bin(word)[-10]
        if sign=="1":
            return -((~field&mask2)+1)
        else:
            return field&mask2
    """
    def extract_signed(self,word: int):
        save = self.extract(word)
        return sign_extend(save,self.bitrange)
